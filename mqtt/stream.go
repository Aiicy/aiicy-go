...
func (c *Client) connect() (*stream, error) {
	...
	if len(c.ops.Subscriptions) > 0 {
		err = c.Subscribe(c.ops.Subscriptions)
		if err != nil {
			return nil, err
		}
	}
...
func (s *stream) send(pkt Packet, async bool) error {
	...
	if ent := s.cli.log.Check(logger.DebugLevel, "client sent a packet"); ent != nil {
		ent.Write(logger.String("pkt", fmt.Sprintf("%v", pkt)))
	}
...
func (s *stream) receiving() error {
	...
	for {
		...
		if ent := s.cli.log.Check(logger.DebugLevel, "client received a packet"); ent != nil {
			ent.Write(logger.String("pkt", fmt.Sprintf("%v", pkt)))
		}
		...
		case *Publish:
			...
			if uerr != nil {
				s.cli.log.Warn("failed to handle publish packet in user code", logger.Error(uerr))
