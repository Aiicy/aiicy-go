...
func NewClient(ops ClientOptions) *Client {
	c := &Client{
		...
		log:   logger.With(logger.String("mqtt", "client"), logger.String("cid", ops.ClientID)),
	}
...
func (c *Client) connecting() error {
	...
	for {
		if !next.IsZero() {
			...
			c.log.Info("next reconnect", logger.Any("at", next), logger.Any("attempt", bf.Attempt()))
		}
		if stream != nil {
			stream.close()
			stream = nil
...
func (c *Client) onError(msg string, err error) {
	...
	c.log.Error(msg, logger.Error(err))
